(function () {
    ...
    function bindViewerGestures(root) {
        ...
        modal.addEventListener('touchstart', function (event) {
            if (!Viewer.active || !event.touches || event.touches.length !== 1) {
                resetGesture();
                return;
            }
            ...
            startTime = Date.now();
            tracking = true;
            closeCandidate = startY >= rect.bottom - Math.min(120, rect.height * 0.2);
        }, false);
        ...
        modal.addEventListener('touchend', function (event) {
            ...
            var horizontal = Math.abs(dx) >= 55 && Math.abs(dx) > Math.abs(dy) * 1.2;
            var closing = closeCandidate && dy <= -75 && Math.abs(dy) > Math.abs(dx) * 1.15;
            resetGesture();

            if (elapsed > 900) return;
            if (horizontal) {
                if (dx < 0 && Viewer.index < Viewer.list.length - 1) Viewer.index++;
                else if (dx > 0 && Viewer.index > 0) Viewer.index--;
                else return;
                drawViewer();
            } else if (closing) {
                closeViewer();
        ...
        Viewer.root = $(
            '<div class="uac-viewer-overlay">' +
                '<div class="uac-viewer-modal">' +
                    ...
                    '<div class="uac-viewer-swipe-handle" aria-hidden="true"><span></span></div>' +
                '</div>' +
            '</div>'
        );

        $('body').append(Viewer.root);
        bindViewerGestures(Viewer.root);
        setTimeout(function () {
    ...
    function addStyles() {
        ...
        style.textContent =
            ...
            '.uac-viewer-text{padding:28px 24px;overflow-y:auto;flex:1;min-height:0;font-size:var(--uac-viewer-font,1.25em);line-height:1.62;color:rgba(255,255,255,.95);white-space:normal}' +
            ...
            '.uac-viewer-swipe-handle{display:none;height:24px;flex:0 0 24px;align-items:center;justify-content:center;touch-action:none}' +
            '.uac-viewer-swipe-handle span{display:block;width:44px;height:4px;border-radius:4px;background:rgba(255,255,255,.28)}' +
            '@media (max-width:900px){.uac-viewer-overlay{padding:10px}.uac-viewer-modal{height:90vh;border-radius:26px}}' +
            '@media (hover:none) and (pointer:coarse) and (max-width:900px){' +
                '.uac-viewer-overlay{align-items:flex-end;padding:8px;padding-bottom:max(8px,env(safe-area-inset-bottom))}' +
                '.uac-viewer-modal{width:100%;max-width:100%;height:92vh;height:92dvh;border-radius:26px;touch-action:pan-y}' +
                '.uac-viewer-arrow{display:none}' +
                '.uac-viewer-head{padding:14px 16px}' +
                '.uac-viewer-text{padding:24px 20px}' +
                '.uac-viewer-swipe-handle{display:flex}' +
            '}';
